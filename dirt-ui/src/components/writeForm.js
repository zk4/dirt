...
export default (props) => {
  ...
  const [fieldName, setFieldName] = useState("com.zk.dirt.entity.MetaType");

  const [createColumns, setCreateColumns] = useState(columns);
  ...
  useEffect(
    () => {
      (async () => {
        d = await network.getDirtFieldTypeAsync({
        ...
        if (actionRef.current)
          actionRef.current.setFieldValue('columnName', '')
        ...
        let createColumns = columns.map(column => {
          ...
          const {key: columnKey, idOfEntity, relation, valueType} = column
          // 清空 filed，如果是 dependency
